...
import { type ButtonColors, type ButtonIconPositions, type ButtonProps, type ButtonSizes } from './types';
...
export function buttonBlockStyles(block?: boolean, iconPosition: ButtonIconPositions = 'left') {
  return [block ? 'flex w-full' : 'inline-flex', iconPosition === 'right' ? 'flex-row-reverse' : 'flex-row'];
}
...
export function buttonIconClasses(size?: ButtonSizes, iconPosition: ButtonIconPositions = 'left', iconOnly = false) {
  switch (size) {
    case 'xs': {
      return [
        ...(iconOnly ? ['-ml-0.5 -mr-0.5'] : [iconPosition === 'right' ? 'ml-2 -mr-0.5' : '-ml-0.5 mr-2']),
        'h-4 w-4',
      ];
    }
    case 'sm': {
    ...
    case 'md': {
      return [...(iconOnly ? ['-ml-1 -mr-1'] : [iconPosition === 'right' ? 'ml-2 -mr-1' : '-ml-1 mr-2']), 'h-5 w-5'];
    }
    case 'lg': {
      return [...(iconOnly ? ['-ml-1 -mr-1'] : [iconPosition === 'right' ? 'ml-3 -mr-1' : '-ml-1 mr-3']), 'h-5 w-5'];
    }
    case 'xl': {
      return [...(iconOnly ? ['-ml-1 -mr-1'] : [iconPosition === 'right' ? 'ml-3 -mr-1' : '-ml-1 mr-3']), 'h-5 w-5'];
    }
