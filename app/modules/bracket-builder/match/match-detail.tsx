...
import { type MatchSettingsData } from '../types/builder-types';
    ...
    <div className="overflow-hidden">
      <div className="flex items-center justify-between p-4">
        <div className="min-w-0 flex-1">
          <h3 className="text-md truncate font-semibold">Match #{index + 1}</h3>
        </div>
      ...
      <SimpleBar>
        <div className="space-y-2 px-4 pt-2 pb-4">
          {match.spots.map((item, spotIndex) => (
            <SpotDetail key={spotIndex} index={spotIndex} matchIndex={index} roundIndex={roundIndex} spot={item} />
          ))}
        </div>
      </SimpleBar>
    </div>
